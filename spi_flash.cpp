...
//    ProgressInit();
	...
	while (len_left)
	{
        ...
        if (!SPIWriteThenRead(op, 5, buf + len_read, len_to_read)) //CmdSize() + 1
            return false;
...
bool FlashSinglePageProgram(unsigned int addr, unsigned char *buff, unsigned int len)
{
    WriteEnable();
    unsigned char op[5];

    op[0] = SPI_CMD_PAGE_PROG;
    AddrToCmd3(addr, &op[1]); //AddrToCmd

    if(!SPIWriteTwo(op, 4, buff, len))
        return false;
...
//    ProgressInit();
	...
	while (bytes_written < len)
	{
        ...
        bytes_to_write = len;//min(bytes_left, PAGE_SIZE - (dst % PAGE_SIZE));
